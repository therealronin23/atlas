...
def test_clean_gateable_kind_falls_through_to_normal_decider(
    ...
    action = _mcp_adopt_action()  # sensitivity="high" -> HumanDecider exige aprobación igual
    task = Task(intent="adoptar ai.adeu/adeu", source=TaskSource.INTERNAL)
    verdict, _ = orch._consult_decider(action, task)
    # clean -> pasa al decisor existente sin cambios; HumanDecider por defecto
    # sigue exigiendo aprobación para sensitivity=high (invariante intacto).
