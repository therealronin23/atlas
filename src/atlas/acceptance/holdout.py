...
class HoldoutProvenance(_FrozenStrictModel):
    """Versioned, content-bound holdout provenance without the protected case."""
...
class EvaluatorIsolationPolicy(_FrozenStrictModel):
    """A declarative candidate-denied policy owned by one evaluator identity."""
...
class CandidateHoldoutAccessDenial(_FrozenStrictModel):
    """A denial record deliberately containing no protected corpus or evaluator data."""
...
class ProtectedHoldoutAccessBoundary:
    """Return denials only; this seam is not an authentication or authority system."""
    ...
    def deny_candidate_access(
        ...
        """Revalidate and deny every candidate request without revealing a holdout."""
...
class HoldoutContaminationFinding(_FrozenStrictModel):
    """One evidence-referenced contamination observation for a named holdout."""
...
class HoldoutContaminationChecker:
    """Record contamination without changing an evaluator's criteria or verdict."""
