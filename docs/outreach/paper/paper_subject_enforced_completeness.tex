...
and the inspection log. The reference implementation comprises 1997 tests and is
...
\textbf{The Attacker Moves Second}~\cite{attacker2024} shows that stronger adaptive
attacks bypass the large majority of evaluated defences. This supports our decision not
...
\textbf{Auditable Agents}~\cite{auditableagents2026} (Nian et al., 2026) proposes a
framework for the auditability of agent execution (tool calls and state transitions). It
does not provide subject-verifiable completeness against an operator that omits entries ---
the case our mechanism targets.

\textbf{Aegis}~\cite{aegis2026} (Mazzocchetti, 2026) provides cryptographic runtime
governance for autonomous agents, enforcing behavioral and policy compliance at execution
time. Aegis addresses \term{behavioral compliance} (did the agent follow rules?), not
completeness of an operator-side inspection log. The two mechanisms are complementary.
...
\textbf{Future directions.} Three extensions would broaden applicability without
altering the core guarantee. (a)~\textit{Standards interoperability}: encoding the
co-signed records as COSE Signed Statements~\cite{scitt-arch} would let the mechanism
interoperate with the emerging SCITT transparency-service ecosystem rather than use a
bespoke format. (b)~\textit{Agent-chain completeness}: combining subject-enforced
completeness with receiver-side counter-signing~\cite{sello2026} would extend the
guarantee from a single subject--operator pair to multi-hop agent workflows, where each
hop attests what it received. (c)~\textit{Consumer-device attestation}: binding the
subject's signing key to mobile secure elements (e.g.\ hardware-attested receipts as in
licensing work~\cite{aegon2026}) would carry the device-bound assumption from servers to
end-user phones.
...
All claims are anchored in 1997 automated tests, mypy \texttt{-{}-strict} clean.

This paper was prepared with AI assistance. The author conceived the core mechanism
and independently verified all claims, citations, and results.
