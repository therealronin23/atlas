...
| MCP trunk | MCP Surface | ACTIVO | Design says MCP is interface/surface; trunk resources measured | `docs/design/mcp_trunk_portable.md` | Interoperability surface, not the whole ecosystem | Keep as surface; classify skills/prompts/APIs separately |
...
| PyYAML dependency floor | Governance | SELLADO | `pyyaml>=6.0.3` accepted; targeted tests/mypy and full verification pass | `pyproject.toml`, `WORK_LEDGER.md` | Dependency floor declaration, not a new dependency | Keep floor unless a dependency audit requires a newer one |
