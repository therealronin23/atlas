...
```bash
PYTHONPATH=src .venv-redteam/bin/python scripts/redteam/garak_campaign.py --attacks 60 --benign 40 --out docs/redteam_campaign_report.md
PYTHONPATH=src .venv-redteam/bin/python scripts/redteam/generalization_curve.py --embedder hf --threshold 0.7 --out docs/immune_generalization_curve.md
CRESCENDO_OUT=docs/pyrit_crescendo_report.md PYTHONPATH=src .venv-redteam/bin/python scripts/redteam/pyrit_crescendo.py
TEACHER_PROVIDER=nvidia DEBATE_OUT=docs/frontier_debate_report.md PYTHONPATH=src .venv-redteam/bin/python scripts/redteam/frontier_debate.py
```
...
- Los reportes generados (`docs/*report*.md`, `docs/immune_generalization_curve.md`) son
